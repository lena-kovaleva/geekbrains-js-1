...
function guessNumber(){
	...
	let stepIsEntering = true;
	while (stepIsEntering) {
		result = prompt("Введите номер хода для вывода (-1 - выйти):", 0);

		if (parseInt(result) == -1) {
			stepIsEntering = false;
		}
		else if (parseInt(result) < 1 || parseInt(result) > steps.length || isNaN(parseInt(result))) {
			alert("Такого хода нет!");
		}
		else {
